...
from src.components import QCNNComponent, QNNComponent, QIComponent
...
class CNN_QCNN_CNN(nn.Module):
    def __init__(self, embedding_type: str, board_size=3, n_qubits=10):
        ...
        self.linear2 = nn.Linear(self.n_qubits//2, self.board_size**2, bias=True)
    ...
    def __init__(self, embedding_type: str, ansatz_type: str, board_size=3, n_qubits=7, feature_map_reps=1, ansatz_reps=1):
        ...
        self.qnn = SamplerQNN(
            sampler=Sampler(
                backend=set_backend.backend,
                options={'shots':1000}
            )if REAL_DEVICE else BackendSampler(
                backend=set_backend.backend,
                options={'shots':1000, 'seed_simulator':SEED}),
            circuit=self.isa_circuit,
...
class QCNN(nn.Module):
    def __init__(self, embedding_type: str, param_weight=1.0, board_size=3, state_weight=1):
        super().__init__()
        self.board_size=board_size
...
class CNN_QCNN_CNN_for_network(nn.Module):
    def __init__(
        ...
        self.circuit, feature_map_parameters, ansatz_parameters = \
            QIComponent(self.n_qubits).make_qcnn_circuit(
                embedding_type=embedding_type,
...
class CNN_QNN_CNN_sampler_for_network(nn.Module):
    def __init__(
        ...
        self.circuit, feature_map_parameters, ansatz_parameters = \
            QIComponent(self.n_qubits).make_qnn_circuit(
                embedding_type=embedding_type,
                ansatz_type=ansatz_type,
                network_model=network_model,
                feature_map_reps=feature_map_reps,
