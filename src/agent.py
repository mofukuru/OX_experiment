...
class CQCAgent_network(Agent):
    def __init__(self, type = 1, player=1, board_size=3, network=None, num_of_qubit=18, featuremap_reps=1, ansatz_reps=1):
        super().__init__(player)
        self.discount = 0.9
        self.epsilon = 0.1
        self.board_size = board_size
        ...
        if network == "CQCCNN_Z_network":
            NN = CQCCNN_Z_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit)
        elif network == "CQCCNN_ZZ_network":
            NN = CQCCNN_ZZ_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit)
        elif network == "CQCCNN_T_network":
            NN = CQCCNN_T_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit)
        elif network == "CQCCNN_H_network":
            NN = CQCCNN_H_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit)
        elif network == "CQCNN_sampler_ZR_network":
            NN = CQCNN_sampler_ZR_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_ZZR_network":
            NN = CQCNN_sampler_ZZR_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_TR_network":
            NN = CQCNN_sampler_TR_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_HR_network":
            NN = CQCNN_sampler_HR_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_ZE_network":
            NN = CQCNN_sampler_ZE_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_ZZE_network":
            NN = CQCNN_sampler_ZZE_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_TE_network":
            NN = CQCNN_sampler_TE_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_sampler_HE_network":
            NN = CQCNN_sampler_HE_network(type=type, board_size=self.board_size, n_qubits=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_ZR_network":
            NN = CQCNN_estimator_ZR_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_ZZR_network":
            NN = CQCNN_estimator_ZZR_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_TR_network":
            NN = CQCNN_estimator_TR_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_HR_network":
            NN = CQCNN_estimator_HR_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_ZE_network":
            NN = CQCNN_estimator_ZE_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_ZZE_network":
            NN = CQCNN_estimator_ZZE_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_TE_network":
            NN = CQCNN_estimator_TE_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        elif network == "CQCNN_estimator_HE_network":
            NN = CQCNN_estimator_HE_network(type=type, board_size=self.board_size, num_of_qubit=num_of_qubit, featuremap_reps=featuremap_reps, ansatz_reps=ansatz_reps)
        else:
            print("no networks")
            exit(1)
...
class QCNNAgent_network(Agent):
    def __init__(self, type=1, player=1, board_size=3, network=None):
        super().__init__(player)
        self.discount = 0.9
        self.epsilon = 0.1
        self.board_size = board_size
        ...
        if network == "QCNN_Z_network":
            NN = QCNN_Z_network(type=type, board_size=self.board_size)
        elif network == "QCNN_ZZ_network":
            NN = QCNN_ZZ_network(type=type, board_size=self.board_size)
        elif network == "QCNN_T_network":
            NN = QCNN_T_network(type=type, board_size=self.board_size)
        elif network == "QCNN_H_network":
            NN = QCNN_H_network(type=type, board_size=self.board_size)
        else:
            print("no networks")
            exit(1)
