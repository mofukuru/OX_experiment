from src_no_seed.tictactoe import TicTacToe
import tqdm
...
class Env:
    ...
    def collect_elorate(self, win_count, draw_count, iters, rate_update):
        ...
        new_rate = self.rate[iters] + 1*(win_count - (rate_update-draw_count)*W)
    ...
    def game_exec(self, train=False, visualize=True, num_of_game=0):
                ...
                if self.tictactoc.is_valid_action(action, player=1):
                    ...
                    if train and not self.agent1.stop:
                        ...
                        if action is not None:
                            self.loss_num = self.agent1.update(self.tictactoc, state, action, reward, num_of_game)
            ...
            while True:
                ...
                if self.tictactoc.is_valid_action(action, player=-1):
                    ...
                    if train and not self.agent2.stop:
                        ...
                        if action is not None:
                            self.loss_num = self.agent2.update(self.tictactoc, state, action, reward, num_of_game)
