...
from src_no_seed.network_re import (
    CCNN,
    CCNN2,
    CNN_instead_sampler,
...
class CNNAgent(Agent):
    def __init__(self, player=1, board_size=3, network=None, n_qubits=1):
        ...
        if network == "CNN_instead_sampler":
            ...
            # NN = CQCNN_sampler_HE(board_size=self.board_size, num_qubits=n_qubits, featuremap_reps=1, ansatz_reps=1)
        if network == "CNN_instead_estimator":
            NN = CNN_instead_estimator(n_qubits=n_qubits)
            for param in NN.linear3.parameters():
               param.requires_grad = False
            # NN = CQCNN_estimator_ZR(board_size=self.board_size, num_of_qubit=n_qubits, featuremap_reps=1, ansatz_reps=1)
        self.NN = NN
    ...
    def update(self, tictactoc, state, action, reward):
        ...
        # print("PARAMS 2")
        # print(self.optimizer.param_groups[0]["params"][2])
        return self.loss_v
        ...
        with torch.no_grad():
            ...
            for i in range(iter):
                if self.noised and random.random() < alpha:
                    self.optimizer.param_groups[0]["params"][2][i] = torch.normal(mean=self.optimizer.param_groups[0]["params"][2][i].item(), std=1, size=(1,1))
